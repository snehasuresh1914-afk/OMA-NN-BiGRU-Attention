...
def load_data(path):
    ...
    def split(dataset):
        ...
        xd = xg.values.reshape(xg.shape[0], 5, int(xg.shape[1]/5))
        y = dataset.iloc[:, 104:]
        return xd, xi, y
...
if __name__ == "__main__":
    ...
    if not os.path.exists("Cost_Building.csv"):
        print("Dataset not found. Please provide Cost_Building.csv")
        exit()

    # Now safe to load
    data = load_data("Cost_Building.csv")
